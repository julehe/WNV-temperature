...
generated quantities {
  ...
  // posterior expected temperature response per species (for Cx. pip. (species 7) as an example)
  matrix[Nexp, N_new] f_new_exp;
  for (m in exp_id) {
    for (n in 1:N_new) {
      f_new_exp[m, n] = briere(x_new[n], q[7]*exp_q_sc[m], Tmin[7]+exp_Tmin_sc[m], Tmax[7]+exp_Tmax_sc[m]);
    }
  }
  
  // posterior expected temperature response new species 
  vector[N_new] f_new_species;
  real Tmax_new;
  real Tmin_new;
  real q_new;
  ...
  for (n in 1:N_new) {
      f_new_species[n] = briere(x_new[n], q_new, Tmin_new, Tmax_new);
  }
}
